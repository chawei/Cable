...
-(NSURL*)extractYouTubeURLFromFile:(NSString *)html error:(NSError *__autoreleasing *)error {
    ...
    if (videos.count > 0) {
        ...
        NSString *streamURLStr = [streamURL stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        //NSLog(@"streamURLStr: %@",streamURLStr);
        
        NSError *errorForRegex = nil;
        NSRegularExpression *httpRegex = 
          [NSRegularExpression 
            regularExpressionWithPattern:@"^(http|https)://"
                                 options:NSRegularExpressionCaseInsensitive
                                   error:&errorForRegex];
        NSTextCheckingResult *match = [httpRegex firstMatchInString:streamURL options:0 range:NSMakeRange(0, [streamURL length])];
        ...
        if (match.numberOfRanges > 0) {
          ...
          [regex stringByReplacingMatchesInString:streamURLStr
                                          options:0
                                            range:NSMakeRange(0, [streamURLStr length])
                                     withTemplate:@""];
          ...
          NSString *modifiedString2 = 
          [regex2 stringByReplacingMatchesInString:modifiedString
                                          options:0
                                            range:NSMakeRange(0, [modifiedString length])
                                     withTemplate:@"&"];
        
          //NSLog(@"modifiedString2: %@",modifiedString2);
        
          NSError *error3 = nil;
          NSRegularExpression *regex3 = 
            [NSRegularExpression 
              regularExpressionWithPattern:@"&type=.+"
                                   options:NSRegularExpressionCaseInsensitive
                                     error:&error3];
        
          NSString *modifiedString3 = 
          [regex3 stringByReplacingMatchesInString:modifiedString2
                                          options:0
                                            range:NSMakeRange(0, [modifiedString2 length])
                                     withTemplate:@""];
        
          //NSLog(@"modifiedString3: %@",modifiedString3);
          return [NSURL URLWithString:modifiedString3];
        }
    }
